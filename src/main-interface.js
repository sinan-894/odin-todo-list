...
import {
  projectList,
  sortToDatesAndGetArrays,
  createProjectListManager,
...
export function mainInterface() {
  ...
  return Object.assign(
    {},
    divContainer(header, sideBar, mainContent, dialog.tag),
    displayProject(),
  );
}
...
function displayProject() {
  ...
  const displayProjectInParent = () => {
    if (currentProject == "Today") {
      const taskList = manageProjectList.sortTaskList(
        daysCatogory.getTodayArray(),
      );
      display(taskList, false, false);
    } else if (currentProject == "Tommarow") {
      const taskList = manageProjectList.sortTaskList(
        daysCatogory.getTommarrowArray(),
      );
      display(taskList, false, false);
    } else if (currentProject == "This Week") {
      const taskList = manageProjectList.sortTaskList(
        daysCatogory.getThisWeekArray(),
      );
      display(taskList, false, false);
  ...
  const { addProjectsNameToSideBar } = displayProjectSidebar(
  ...
  const getProjectTaskDataIntoDiv = (taskList) => {
    ...
    taskList.forEach((task) => {
      ...
      dom.makeParent(
        taskDiv,
        createDeleteTaskButton(taskListDiv, taskDiv, task),
      );
...
function displayProjectButtonLogic(
  getCurrentProject,
  displayProjectInMain,
  addProjectsNameToSideBar,
) {
  const {
    modifyFormSubmitButtonText,
    onSubmitButtonPressForCreatingTask,
    onSubmitButtonPressForEditingTask,
  } = mainDialog(getCurrentProject, displayProjectInMain);
  ...
  return Object.assign(
    { addTaskContainer, createEditTaskButton, deleteProjectButton },
    deleteTask(),
  );
}
...
function mainDialog(getCurrentProject, displayProjectInMain) {
  ...
  const onSubmitButtonPressForEditingTask = (task) => {
    return () => {
      ...
      manageLocalStorage.updateLocalStorage(
        manageProjectList.findProjectOftheTask(task),
      );
      displayProjectInMain();
    };
  };
