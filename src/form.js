...
function createTaskFormHandler() {
  ...
  const createSubmitButton = () => {
    const submitButton = dom.createTag(
      "button",
      "task-submit-button",
      null,
      "submit",
  ...
  return Object.assign(
    ...
    extractDataFromTheFormAndCreatObject(tagArray),
  );
}
...
export function createDialog() {
  const {
    createForm,
    modifyFormSubmitButtonEventHandler,
    setFormValue,
    getDataAndStoreToProjectList,
    getData,
...
function createInputTag(tagName, label, classes = []) {
  ...
  classes.forEach((styleClass) => {
    inputTag.classList.add(styleClass);
  });
...
function createTextAreaTag(title, classes = [], isMandatory = false) {
  const { inputTag, label, getValue } = createInputTag(
    "textarea",
    title,
    classes,
  );
